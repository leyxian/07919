...
function PageOnShutdown(){
	...
	if(!array_empty($msg)){
		...
		MooWriteFile(dirname(__file__)."/data/zha_collect.txt",$str,"a+");
		
	}
	MooWriteFile(dirname(__file__)."/data/zha_collect.txt",'[script shutdown]',"a+");
}
...
if(!empty($status)){
	...
	mc_set($key,$key.' running!',86400);
}unset($status);

MooWriteFile(dirname(__file__)."/data/zha_collect.txt","start-->,","a+");

$file = dirname(__file__).'/data/collect_list.txt';

$num = isset($_GET['num'])?(int)trim($_GET['num']):1;
$num = max(1,$num);
...
while($num > 0){
	...
	if($i>=470){unlink($file)}
	MooWriteFile($file,','.$i,"a+");
	if(!isset($collectList[$i])) continue;
	$list = array_merge($list,$collectList[$i]);
	unset($str,$i,$arr);
	$num--;
}
if(array_empty($list)) {
	MooWriteFile(dirname(__file__)."/data/zha_collect.txt",'[no collect list],',"a+");
	exit("no collect list!");
}
foreach($list as $k=>$v){
	run('collect');
	$a = explode(',',$k);
	$url = 'http://search.zhenai.com/search/getfastmdata.jsps?condition=2&photo=1&agebegin=18&ageend=26&workcityprovince='.$a[0].'&workcitycity='.$a[1];
	foreach($v as $ke=>$va){
		$mc_num = $_MooClass['MooMySQL']->getOne("select count(*) as c from {$dbTablePre}members_collect where province = {$a[0]} and city= {$a[1]} and gender = {$ke}");
		if($mc_num['c'] > 500){
			continue;
		}elseif($mc_num['c'] > 0){
			$va = 500 - $mc_num['c'];
...
if(!isset($urlArr) ){
	MooWriteFile(dirname(__file__)."/data/zha_collect.txt","no page list,","a+");
	exit("no page list");
}
...
$info['url'] = 'http://profile.zhenai.com/login/loginactionindex.jsps';
$info['peferer'] = 'http://profile.zhenai.com/';
$info['loginData']['loginInfo'] = '13696545523';
$info['loginData']['password'] = '120120';
$info['loginData']['loginmode'] = 2;
$info['loginData']['rememberpassword'] = 1;
$contents = $mc->webLogin($info);
$i = 0;
while(!$contents){
	$i++;
	if($i>3){
		MooWriteFile(dirname(__file__)."/data/zha_collect.txt",'[login false],',"a+");
		exit('login false');
	}
	$contents = $mc->webLogin($info);
}
...
foreach($urlArr as $k=>$v){
	...
	parse_str($arr['query']);
	$info['url'] = $v;
	$contents = $mc->webVisit($info);
	$i = 0;
	while(!$contents){
		$i++;
		if($i>3){ 
			MooWriteFile(dirname(__file__)."/data/zha_collect.txt","{$v}['false'],","a+");
			continue 2;
		}
		$contents = $mc->webVisit($info);
	}unset($info);
	...
	if(array_empty($list)){
		MooWriteFile(dirname(__file__)."/data/zha_collect.txt","{$v}no list,","a+");
		continue;
	}
	foreach($list as $v){
		$lisArr[] = $v[0];
	}unset($list);
	if(array_empty($lisArr)){continue;}
	$lisArr = array_unique($lisArr);
	foreach($lisArr as $k=>$v){
		//脚本控制器
		run('collect');
		//判断是否存在
		if(dbHas('members_collect',array('source' => $v) ) ){
			MooWriteFile(dirname(__file__)."/data/zha_collect.txt","has,","a+");continue;
		}
		$time = time();
		$info['url'] = $v;
		$contents = $mc->webVisit($info);
		...
		//日志
		MooWriteFile(dirname(__file__)."/data/zha_collect.txt","{$uid},","a+");
		if($_SERVER['HTTP_HOST']!='www.07919.com') {mc_unset($key);pr($uid,1);}
		unset($time,$contents,$str,$data,$mbData,$v,$pc,$mz,$ip,$res,$imgurl,$photosname,$pathParts,$photos,$date,$uid,$introduce);
	}unset($lisArr,$gender);
}
mc_unset($key);
$_MooClass['MooMySQL']->close();
unset($urlArr,$nickData,$im,$key,$mc,$_MooClass,$dbTablePre);
unlink($cookie_file);
MooWriteFile(dirname(__file__)."/data/zha_collect.txt","<--end,","a+");
exit();
